...
class Tagger_model
{
	...
	public function get_tags($limit=FALSE, $tag_id=FALSE)
	{
		...
		ee()->db->order_by('tag_name', 'asc');
		if ($tag_id != FALSE) ee()->db->where('tag_id', $tag_id);
		if ($limit != FALSE) ee()->db->limit($limit);
	...
	{
		ee()->db->select('group_id, group_title, group_name, group_desc');
		ee()->db->from('exp_tagger_groups');
		if ($group_id != FALSE) ee()->db->where('group_id', $group_id);
		ee()->db->where('site_id', $this->site_id);
		$query = ee()->db->get();
	...
	{
		ee()->db->select('group_id, tag_id');
		ee()->db->from('exp_tagger_groups_entries ');
		$query = ee()->db->get();
